...
void protobuf_InitDefaults_cncstatebuf_2eproto_impl() {
  ...
  _StateBuf_default_instance_.get_mutable()->offset_fixture_ = const_cast< ::CncRemote::Axes*>(
      ::CncRemote::Axes::internal_default_instance());
  _StateBuf_default_instance_.get_mutable()->homed_ = const_cast< ::CncRemote::BoolAxes*>(
...
StateBuf::StateBuf(const StateBuf& from)
  ...
  if (from.has_homed()) {
    homed_ = new ::CncRemote::BoolAxes(*from.homed_);
  } else {
    homed_ = NULL;
  }
...
// @@protoc_insertion_point(message_clear_start:CncRemote.StateBuf)
  if (_has_bits_[0 / 32] & 15u) {
    ...
    if (has_homed()) {
      GOOGLE_DCHECK(homed_ != NULL);
      homed_->::CncRemote::BoolAxes::Clear();
    }
  }
  if (_has_bits_[0 / 32] & 240u) {
    ::memset(&lin_unit_scale_, 0, reinterpret_cast<char*>(&machine_connected_) -
      reinterpret_cast<char*>(&lin_unit_scale_) + sizeof(machine_connected_));
  }
  if (_has_bits_[8 / 32] & 65280u) {
    ::memset(&paused_, 0, reinterpret_cast<char*>(&spindle_state_) -
      reinterpret_cast<char*>(&paused_) + sizeof(spindle_state_));
  }
  if (_has_bits_[16 / 32] & 196608u) {
    ::memset(&spindle_speed_, 0, reinterpret_cast<char*>(&flood_) -
      reinterpret_cast<char*>(&spindle_speed_) + sizeof(flood_));
  }
  _has_bits_.Clear();
  _internal_metadata_.Clear();
}
